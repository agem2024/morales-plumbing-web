...
<body>
    ...
    <div class="service-detail-container animate-in">
        ...
        <div class="comic-section" style="margin-top: 60px;">
            <h2 class="tech-spec-header" data-i18n="eng_[[KEY]]_comic_title" style="text-align: center;">Metodología (Storyboard)</h2>
...
for p in pages:
    ...
    content = content.replace('[[STYLE]]', base_style)
    content = content.replace('[[KEY]]', p['key'])
    content = content.replace('[[CODE]]', p['code'])
    content = content.replace('[[ICON]]', p['icon'])
    content = content.replace('[[COMIC]]', p['comic'])
