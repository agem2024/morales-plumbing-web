...
diy_path = os.path.join('docs', 'diy_hub.html')
with open(diy_path, 'r', encoding='utf-8') as f:
    diy_content = f.read()

# 1. Update diagData in generateDiagnosticReport
new_diag_data = """
...
diy_content = re.sub(r'(smell:.*?})', r'\1,\n' + new_diag_data, diy_content)

# 2. Update diyTrans for EN and ES
new_trans_en = """
...
"""
diy_content = diy_content.replace(r'"sm3": "If the odor persists, the roof vent pipe may be blocked, requiring specialized equipment."', r'"sm3": "If the odor persists, the roof vent pipe may be blocked, requiring specialized equipment.",' + new_trans_en)
diy_content = diy_content.replace(r'"sm3": "Si el olor persiste, es posible que la tubería de ventilación en el techo esté bloqueada, requiriendo equipo especializado."', r'"sm3": "Si el olor persiste, es posible que la tubería de ventilación en el techo esté bloqueada, requiriendo equipo especializado.",' + new_trans_es)

# 3. Update getIssuesData Panels
new_issues = """
                tankless_flush: {
...
diy_content = re.sub(r'(smell: \{.*?\})', r'\1,\n' + new_issues, diy_content, flags=re.DOTALL)

# 4. Update issuesLinks
new_links = """
            tankless_flush: [
...
diy_content = re.sub(r'(smell: \[.*?\])', r'\1,\n' + new_links, diy_content, flags=re.DOTALL)

with open(diy_path, 'w', encoding='utf-8') as f:
    f.write(diy_content)
print("diy_hub.html updated")
