...
## 2. CÓMO FUNCIONA EL DISPATCHER (Enrutador de IA)
Joe no funciona con un solo servidor, utiliza un sistema de **"Waterfall Fallback"** (cascaída de respaldo) para asegurar que nunca se caiga.
...
1. **Nivel 1: Máquina de Estados Local (Booking Mode)**
   ...
   - Entra en un ciclo de 7 preguntas pre-programadías para no gastar API keys y garantizar la recolección perfecta de los 7 datos necesarios (Nombre, Tel, Dirección, Servicio, Fecha, Hora, Notas).
...
4. **Nivel 4: Client-Side Fallback (Gemini API Keys)**
   - Si todos los servidores fallan, el código usa un arreglo rotativo de API keys de Gemini cargadías directamente en el frontend. Cambia de llave si una supera su cuota.
...
* **`[ACTION_SET_PROFILE] {...}`**
  - Guíarda los datos del cliente (nombre, teléfono, email) en su perfil local.
...
4. **Validación:** El sistema genera un código de 6 dígitos (ej. `839102`), lo guíarda en Firestore (colección `confirmation_codes`), y envía WhatsApps al Admin y al Cliente.
...
- **Historial:** Se envían los últimos mensajes de contexto (`joeHistory`) que están guíardados en `localStorage` (memoria a corto plazo).
